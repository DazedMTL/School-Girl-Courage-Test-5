...
Moghunter.pixiFilter_BlurLoop = String(
  Moghunter.parameters["Blur Loop"] || "false"
);
Moghunter.pixiFilter_PixelateLoop = String(
  Moghunter.parameters["Pixelate Loop"] || "false"
);
...
// * PluginCommand
//==============================
var _alias_mog_pixifilters_pluginCommand =
...
Game_Interpreter.prototype.setFilterInt = function (command, args) {
  ...
  if (command === "filter_clear") {
    $gameSystem.clearPixiFData();
  }
...
// ** Spriteset Map
//=============================================================================
var _mog_pxfilter_sprtsetMap_createLowerLayer =
...
Spriteset_Map.prototype.update = function () {
  ...
  if ($gameSystem._pixiFilterRefresh) {
    this.refreshFilterChar();
  }
};
...
Spriteset_Map.prototype.setCharFilterData = function (char) {
  if (char._character._filterData[0]) {
    char.filters =
      char._character._filterData[1] >= 0
        ? [this.setPixiFilter(char._character._filterData[1])]
        : [];
  }
};
...
// * Initialize
//==============================
var _mog_pixifilter_spritesetbase_initialize =
  Spriteset_Base.prototype.initialize;
...
Spriteset_Base.prototype.pixeFilterSetup = function () {
  ...
  this._blurloop =
    String(Moghunter.pixiFilter_BlurLoop) === "true" ? true : false;
  this._pixelateloop =
    String(Moghunter.pixiFilter_PixelateLoop) === "true" ? true : false;
  if (this.pfData()[1] != -1) {
    this.refreshPixiFilterAnimation();
  }
  if ($gameSystem._pixiFilterData[1] != -1) {
    this.refreshPixiFilter();
  }
};
...
Spriteset_Base.prototype.updatePixiFilters = function () {
  if ($gameSystem._pixiFilterEffects[0]) {
    this.refreshPixiFilterAnimation();
  }
  if ($gameSystem._pixiFilterData[0]) {
    this.refreshPixiFilter();
  }
};
