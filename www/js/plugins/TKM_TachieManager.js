...
(function () {
  ...
  $TKMvar.tachie.preloadBitmap = function (tachieNum) {
    ...
    for (var i = 0; i < MaxLayer; i++) {
      ...
      if (partList[i] === 0) {
        ...
        PicData[tachieNum]["bitmap"][i] = ImageManager.loadPicture(
          name + "_" + $TKMvar.tachie.partsNameArr[i] + "_" + partList[i],
          0
        );
      }
    }
  ...
  for (var i = 0; i < $TKMvar.tachie.PicData.length; i++) {
    $TKMvar.tachie.PicData[i]["picNum"] = parseInt(
      param["立ち絵" + (i + 1) + "_ピクチャ番号"],
      10
    );
    $TKMvar.tachie.PicData[i]["x"] = parseInt(
      param["立ち絵" + (i + 1) + "_x座標"],
      10
    );
    $TKMvar.tachie.PicData[i]["y"] = parseInt(
      param["立ち絵" + (i + 1) + "_y座標"],
      10
    );
    $TKMvar.tachie.PicData[i]["reverse"] = parseInt(
      param["立ち絵" + (i + 1) + "_反転"],
      10
    )
      ? true
  ...
  Bitmap.prototype.resizeIfSmallerThan = function (bitemp) {
    // 立ち絵合成に使う
    var changed = false;
  ...
  Sprite_Picture.prototype.loadBitmap = function () {
    if (this._pictureName.substr(0, 10) !== "TKMtachie_") {
      ...
      // ピクチャのname要素の接頭が TKMtachie_ だったら
      // 取得
      var PicData = $TKMvar.tachie.PicData;
      var MaxLayer = $TKMvar.tachie.MaxLayer;

      if (!this.bitmap || !this.bitmap.isTachie) {
        // Tachie専用のbitmapかどうか
      ...
      for (var i = 0; i < PicData.length; i++) {
        if (this._pictureId === PicData[i]["picNum"]) {
          // このSprite_Pictureに対応するPicDataを探す
          HasController = true;
        ...
        for (var i = 0; i < PicData.length; i++) {
          if (PicData[i]["char"] == args[0]) {
            ...
            if (partNum === 0) PicData[i]["bitmap"][layerNum] = null;
              ...
              PicData[i]["bitmap"][layerNum] = ImageManager.loadPicture(
                args[0] +
                  "_" +
                  $TKMvar.tachie.partsNameArr[layerNum] +
                  "_" +
                  partNum,
                0
              );
