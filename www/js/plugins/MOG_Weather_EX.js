...
// ** Game_Interpreter
//=============================================================================
var _alias_mog_weather_ex_pluginCommand =
...
Scene_Base.prototype.hideWeatherSprites = function () {
  if (this._spriteset._WeatherPlane) {
    this._spriteset._WeatherPlane.visible = false;
  }
};
...
Scene_Map.prototype.terminate = function () {
  if (this._spriteset && this._spriteset._weatherSprites) {
    this.recordWeatherEXData();
  }
...
//==============================
var _mog_weather_ex_scMap_snapForBattleBackground =
  Scene_Map.prototype.snapForBattleBackground;
Scene_Map.prototype.snapForBattleBackground = function () {
  if (this._spriteset && this._spriteset._WeatherPlane) {
    this._spriteset._WeatherPlane.visible = false;
  }
  _mog_weather_ex_scMap_snapForBattleBackground.call(this);
  if (this._spriteset && this._spriteset._WeatherPlane) {
    this._spriteset._WeatherPlane.visible = true;
  }
};
...
// * createLowerLayer
//==============================
var _alias_mog_weather_ex_sprtbase_createToneChanger =
  Spriteset_Base.prototype.createToneChanger;
...
Spriteset_Base.prototype.refreshWeatherEX = function () {
  if (this._weatherSprites) {
    this.removeWeatherEXSprites();
  }
  this.setWeatherEXData();
  this._weatherSprites = null;
  $gameSystem._weatherData.needRefresh = false;
  if ($gameSystem._weatherData.mode >= 0) {
    this.createWeatherExSprites();
  } else {
    $gameSystem.clearWeatherEX();
  }
};
...
Spriteset_Base.prototype.needRefreshWeatherEX = function () {
  ...
  if (this._weatherData.mode != $gameSystem._weatherData.mode) {
    return true;
  }
  if (this._weatherData.fileName != $gameSystem._weatherData.fileName) {
    return true;
  }
  if (this._weatherData.power != $gameSystem._weatherData.power) {
    return true;
  }
  if ($gameSystem._weatherData.needRefresh) {
    return true;
  }
  return false;
};
...
SpriteWeatherEX.prototype.needRefreshWeather = function () {
  if (this.x < this.screenLimitX1()) {
    return true;
  }
  if (this.x > this.screenLimitX2()) {
    return true;
  }
  if (this.y < this.screenLimitY1()) {
    return true;
  }
  if (this.y > this.screenLimitY2()) {
    return true;
  }
  return false;
};
...
SpriteWeatherEX.prototype.update = function () {
  ...
  if (!this._cam[2] && $gameParty.inBattle() && Imported.MOG_BattleCamera) {
    this.setCamScreen();
  }
  this.updateEffects();
  this.updatePosition();
