...
Moghunter.titleBackV = [];
Moghunter.titleBackN = [];
Moghunter.titleBackX = [];
Moghunter.titleBackY = [];
Moghunter.titleBackZ = [];
Moghunter.titleBackT = [];
Moghunter.titleBackAN = [];
Moghunter.titleBackAF = [];
Moghunter.titleBackAS = [];
Moghunter.titleBackMode = [];
Moghunter.titleBackB = [];
Moghunter.titleBackW = String(
  Moghunter.parameters["Screen Movement"] || "false"
);
Moghunter.titleBackWS = Number(Moghunter.parameters["Screen Move Speed"] || 1);
Moghunter.titleBackWD = Number(
  Moghunter.parameters["Screen Move Duration"] || 160
);
for (var i = 0; i < Moghunter.titleBackM; i++) {
  Moghunter.titleBackV[i] = String(
    Moghunter.parameters["L" + String(i + 1) + " Visible"] || "false"
  );
  Moghunter.titleBackN[i] = String(
    Moghunter.parameters["L" + String(i + 1) + " File Name"] || "Layer"
  );
  Moghunter.titleBackMode[i] = Number(
    Moghunter.parameters["L" + String(i + 1) + " Mode"] || 0
  );
  Moghunter.titleBackZ[i] = Number(
    Moghunter.parameters["L" + String(i + 1) + " Z Index"] || 0
  );
  Moghunter.titleBackX[i] = Number(
    Moghunter.parameters["L" + String(i + 1) + " X-axis / Scroll"] || 1
  );
  Moghunter.titleBackY[i] = Number(
    Moghunter.parameters["L" + String(i + 1) + " Y-axis / Scroll"] || 1
  );
  Moghunter.titleBackT[i] = Number(
    Moghunter.parameters["L" + String(i + 1) + " Transition Time"] || 0
  );
  Moghunter.titleBackAN[i] = String(
    Moghunter.parameters["L" + String(i + 1) + " Animated"] || "false"
  );
  Moghunter.titleBackAF[i] = Number(
    Moghunter.parameters["L" + String(i + 1) + " Frames"] || 4
  );
  Moghunter.titleBackAS[i] = Number(
    Moghunter.parameters["L" + String(i + 1) + " Animation Speed"] || 20
  );
  Moghunter.titleBackB[i] = Number(
    Moghunter.parameters["L" + String(i + 1) + " Blend Mode"] || 0
  );
}
...
// * Create Background
//==============================
var _mog_title_background_effects_createBackground =
...
Scene_Title.prototype.checkTfl = function () {
  if (this._titleField.x < this.xl1()) {
    this.refreshTfl();
    this._titleField.x = this.xl1();
  }
  if (this._titleField.x > this.xl2()) {
    this.refreshTfl();
    this._titleField.x = this.xl2();
  }
  if (this._titleField.y < this.yl1()) {
    this.refreshTfl();
    this._titleField.y = this.yl1();
  }
  if (this._titleField.y > this.yl2()) {
    this.refreshTfl();
    this._titleField.y = this.yl2();
  }
...
Scene_Title.prototype.update = function () {
  ...
  if (this._backgroundEX && this._backWave) {
    this.updateBackWave();
  }
};
...
TitleBackground.prototype.initialize = function (index, data) {
  ...
  this._index = index;
  this._enable =
    String(Moghunter.titleBackV[this._index]) === "true" ? true : false;
  if (this._enable) {
    this._data = data;
...
TitleBackground.prototype.updateFrames = function () {
  this._anime[4]++;
  if (this._anime[4] < this.frameSpeed()) {
    return;
  }
  this._anime[4] = 0;
  this.refreshFrames();
...
TitleBackground.prototype.refreshFrames = function () {
  this._anime[1]++;
  if (this._anime[1] >= this.maxframes()) {
    this._anime[1] = 0;
  }
  this.setBitmap();
};
...
TitleBackground.prototype.loadBitmap = function () {
  this._anime = [];
  this._anime[0] =
    String(Moghunter.titleBackAN[this._index]) === "true" ? true : false;
  this._anime[1] = 0;
