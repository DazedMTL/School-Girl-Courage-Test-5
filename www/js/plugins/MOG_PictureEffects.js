...
Game_Interpreter.prototype.picEfctSetAni = function (command, args) {
  ...
  if (command === "pic_animated") {
    ...
    $gameScreen.picture(Number(args[1]))._animeData = [
      true,
      frm,
      9999,
      0,
  ...
  if (command === "pic_breath") {
    ...
    var pw2 = 1 + pw2 * 0.1;
    $gameScreen.picture(Number(args[1]))._breathEffect = [
      enable,
      0,
      0,
      0,
  ...
  if (command === "pic_float") {
    ...
    $gameScreen.picture(Number(args[1]))._floatEffect = [
      enable,
      0,
      0,
      0,
  ...
  if (command === "pic_smooth") {
    ...
    var pw2 = pw2 * 0.01;
    $gameScreen.picture(Number(args[1]))._moveEffect = [
      enable,
      0,
      0,
      160,
...
Game_Picture.prototype.show = function (
  ...
  if (this._breathEffect[0]) {
    this._breathEffect[3] = (Math.random() * 0.2).toFixed(2);
  }
...
Game_Picture.prototype.updatePictureEffects = function () {
  ...
  if (this._positionData[0] > 0) {
    this.updatePicPosEfct();
  }
  if (this._moveEffect[0]) {
    this.updateMoveEfct();
  }
...
Game_Picture.prototype.updateMoveEfct = function () {
  this._moveEffect[3]++;
  this._moveEffect[1] = this.movePictureEfc(
    this._moveEffect[1],
    this._moveEffect[4],
    this._moveEffect[7]
  );
  this._moveEffect[2] = this.movePictureEfc(
    this._moveEffect[2],
    this._moveEffect[5],
    this._moveEffect[7]
  );
  ...
  this._moveEffect[4] =
    r === 0
      ? Math.randomInt(this._moveEffect[6])
      : -Math.randomInt(this._moveEffect[6]);
  var r = Math.randomInt(2);
  this._moveEffect[5] =
    r === 0
...
Game_Picture.prototype.updateFloatEffect = function () {
  ...
  if (this._floatEffect[1] === 0) {
    ...
    if (this._floatEffect[3] <= -this._floatEffect[4]) {
      this._floatEffect[1] = 1;
    }
  } else {
    this._floatEffect[3] += this._floatEffect[5];
    if (this._floatEffect[3] >= 0) {
      this._floatEffect[1] = 0;
    }
  }
};
...
Sprite_Picture.prototype.updateFrames = function (picture) {
  ...
  if (picture._animeData[2] < picture._animeData[4]) {
    return;
  }
  picture._animeData[2] = 0;
  this.setFrame(
    picture._animeData[3] * this._picFrames[3],
    0,
    this._picFrames[3],
    this._picFrames[4]
  );
  picture._animeData[3]++;
  if (picture._animeData[3] >= this._picFrames[0]) {
    picture._animeData[3] = 0;
  }
};
...
Sprite_Picture.prototype.updatePicEffect = function () {
  ...
  if (this.picture()._positionData[0] === 0) {
    ...
    if (this.picture()._floatEffect[0]) {
      this.y += this.picture()._floatEffect[3];
    }
  }
};
