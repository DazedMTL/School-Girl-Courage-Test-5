...
(function () {
  ...
  Window_Ticker.prototype.update = function () {
    ...
    this.frameCount++;
    if (this.isOpening) {
      this._backSprite.opacity =
        (this.frameCount / TOGGLE_DURATION) * BACKGROUND_OPACITY;
      this.contentsOpacity = (this.frameCount / TOGGLE_DURATION) * 255;
      this.textX = ((1 - (this.frameCount * 2) / TOGGLE_DURATION) * 100).clamp(
        0,
        100
      );
      ...
      this._backSprite.opacity =
        (1 - this.frameCount / TOGGLE_DURATION) * BACKGROUND_OPACITY;
