...
(function () {
  ...
  StorageManager.copyFile = function (originalPath, targetPath, fileName) {
    node.fs
      .createReadStream(originalPath + fileName)
      .pipe(node.fs.createWriteStream(targetPath + fileName));
  };
  ...
  StorageManager.getBackupPath = function (prefix) {
    var date = new Date();
    return `${prefix}_${date.getFullYear()}-${(date.getMonth() + 1).padZero(
      2
    )}-${date.getDate().padZero(2)}${this.getBackupTimePath()}/`;
  };
  ...
  StorageManager.getBackupTimePath = function () {
    ...
    var date = new Date();
    return `_${date.getHours().padZero(2)}${date.getMinutes().padZero(2)}${date
      .getSeconds()
      .padZero(2)}`;
  };
